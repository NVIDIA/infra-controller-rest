...
type ApiUpdateTenantCapabilitiesRequest struct {
	...
	org                           string
	tenantId                      string
	tenantCapabilityUpdateRequest *TenantCapabilityUpdateRequest
}

// Tenant capability update request
func (r ApiUpdateTenantCapabilitiesRequest) TenantCapabilityUpdateRequest(tenantCapabilityUpdateRequest TenantCapabilityUpdateRequest) ApiUpdateTenantCapabilitiesRequest {
	r.tenantCapabilityUpdateRequest = &tenantCapabilityUpdateRequest
	return r
}

func (r ApiUpdateTenantCapabilitiesRequest) Execute() (*TenantCapability, *http.Response, error) {
	return r.ApiService.UpdateTenantCapabilitiesExecute(r)
}

/*
UpdateTenantCapabilities Update scoped Tenant capabilities

Enable or disable a scoped capability (e.g. `TargetedInstanceCreation`) for a resolved set of Sites.

Org must have a Tenant entity whose ID matches the `tenantId` path param. User must have authorization role with `TENANT_ADMIN` suffix.

Enabling a capability requires the tenant-level entitlement (`Tenant.Config`) to already be set; disabling is always permitted.

An empty `sites` array applies the change to all eligible Sites (the union of Tenant Site memberships and Sites reachable via `Ready` Tenant Accounts), optionally restricted by `infrastructureProviderId`. A non-empty `sites` array scopes the change to the listed Sites.
...
func (a *TenantAPIService) UpdateTenantCapabilitiesExecute(r ApiUpdateTenantCapabilitiesRequest) (*TenantCapability, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPatch
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *TenantCapability
	)
	...
	localVarPath := localBasePath + "/v2/org/{org}/nico/tenant/{tenantId}/capabilities"
	localVarPath = strings.Replace(localVarPath, "{"+"org"+"}", url.PathEscape(parameterValueToString(r.org, "org")), -1)
	...
	// body params
	localVarPostBody = r.tenantCapabilityUpdateRequest
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return localVarReturnValue, nil, err
	}
