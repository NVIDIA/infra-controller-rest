...
func TestAllCommands_HasInstanceUpdateAndReboot(t *testing.T) {
	// Regression guard for nvbug-6124487: the TUI command registry must
	// expose `instance update` (so users can rename, swap OS, rotate ssh
	// key groups, or trigger a reboot) and `instance reboot` (the dedicated
	// reboot abstraction).
