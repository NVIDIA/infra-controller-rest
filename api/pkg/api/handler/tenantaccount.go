...
// @Param tenantId query string false "Filter TenantAccounts by Tenant ID (Provider role only; for Tenant role the tenant is inferred from org membership and this param is ignored). Can be specified multiple times to filter on more than one Tenant ID."
...
func (gatah GetAllTenantAccountHandler) Handle(c echo.Context) error {
	...
	// Optional Provider-side tenantId narrowing filter.
	var filterTenantIDs []uuid.UUID
	if tenantIdQuery := qParams["tenantId"]; len(tenantIdQuery) > 0 {
		gatah.tracerSpan.SetAttribute(handlerSpan, attribute.StringSlice("tenantId", tenantIdQuery), logger)
		for _, tenantID := range tenantIdQuery {
			...
			if serr != nil {
				...
				return cutil.NewAPIErrorResponse(c, http.StatusBadRequest, fmt.Sprintf("Invalid Tenant ID: %s in query", tenantID), nil)
			}
