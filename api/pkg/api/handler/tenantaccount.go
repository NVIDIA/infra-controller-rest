...
func (gatah GetAllTenantAccountHandler) Handle(c echo.Context) error {
	...
	if tenantIdQuery := c.QueryParam("tenantId"); tenantIdQuery != "" {
	...
	// Pre-compute Allocation counts in bounded queries instead of one per row.
	// Group the page's TAs by their Infrastructure Provider and issue one
	// Allocation query per provider, then bucket results by (Provider, Tenant).
	// In the common case the merged page comes from one provider (Provider Admin
	// view) so this collapses to a single query; for Tenant-Admin and dual-role
	// callers with accounts across multiple providers, it is O(providers), not
	// O(accounts).
	type ipTenantKey struct{ IP, Tenant uuid.UUID }
	allocCountByPair := map[ipTenantKey]int{}
	tenantsByProvider := map[uuid.UUID]mapset.Set[uuid.UUID]{}
	for _, ta := range tas {
		if ta.TenantID == nil {
			continue
		}
		set, ok := tenantsByProvider[ta.InfrastructureProviderID]
		if !ok {
			set = mapset.NewSet[uuid.UUID]()
			tenantsByProvider[ta.InfrastructureProviderID] = set
		}
		set.Add(*ta.TenantID)
	}
	allocPage := cdbp.PageInput{Limit: cdb.GetIntPtr(cdbp.TotalLimit)}
	for ipID, tenantSet := range tenantsByProvider {
		ipID := ipID
		als, _, aerr := aDAO.GetAll(ctx, nil, cdbm.AllocationFilterInput{
			InfrastructureProviderID: &ipID,
			TenantIDs:                tenantSet.ToSlice(),
		}, allocPage, nil)
		if aerr != nil {
		...
		for _, al := range als {
			allocCountByPair[ipTenantKey{IP: al.InfrastructureProviderID, Tenant: al.TenantID}]++
		}
	}
	...
	for _, ta := range tas {
		...
		if tmpTa.TenantID != nil {
			allocCount = allocCountByPair[ipTenantKey{IP: tmpTa.InfrastructureProviderID, Tenant: *tmpTa.TenantID}]
		}
...
func (gtah GetTenantAccountHandler) Handle(c echo.Context) error {
	...
	if ta.TenantID != nil {
		cnt, cerr := aDAO.GetCount(ctx, nil, cdbm.AllocationFilterInput{
			InfrastructureProviderID: &ta.InfrastructureProviderID,
